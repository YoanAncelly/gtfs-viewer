...
$(document).ready(function() {
    ...
    $('#test-urls-btn').click(function() {
        testUrls();
    });

    // Edit test URLs button click
    $('#edit-test-urls-btn').click(function() {
        testEditUrls();
    });
...
function loadConfig() {
    $.ajax({
        ...
        error: function(xhr, status, error) {
            showAlert('danger', 'Erreur lors du chargement de la configuration: ' + error);
        }
    });
}
...
function renderSources(config) {
    ...
    if (!config || !config.sources || config.sources.length === 0) {
        sourcesContainer.html('<div class="col-12"><div class="alert alert-warning">Aucune source configurée.</div></div>');
        return;
    }
                ...
                <div class="card source-card ${isActive ? 'active-source' : ''}">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div class="btn-group">
                            ...
                            ${!isActive ? `<button class="btn btn-sm btn-outline-success set-active" data-index="${index}">
...
function testUrls() {
    ...
    $('#test-results').removeClass('d-none').html('<div class="spinner-border text-primary" role="status"><span class="visually-hidden">Chargement...</span></div>');
    ...
    $.ajax({
        ...
        error: function(xhr, status, error) {
            $('#test-results').html(`<div class="alert alert-danger">Erreur lors du test: ${error}</div>`);
        }
    });
}
...
function testEditUrls() {
    ...
    // Show loading indicator
    $('#edit-test-results').removeClass('d-none').html('<div class="spinner-border text-primary" role="status"><span class="visually-hidden">Chargement...</span></div>');
    ...
    $.ajax({
        ...
        error: function(xhr, status, error) {
            $('#edit-test-results').html(`<div class="alert alert-danger">Erreur lors du test: ${error}</div>`);
        }
    });
}
...
function displayTestResults(results, containerId) {
    ...
    for (const feedType in results) {
        ...
        if (result.success === true) {
            ...
            statusMessage = result.error ? `Erreur: ${result.error}` : 
                           (result.status_code ? `Code: ${result.status_code} (Erreur)` : 'Échec');
        }
...
function saveNewSource() {
    ...
    if (!useLocalFiles) {
        ...
        if (!tripUpdateUrl && !vehiclePositionUrl && !alertUrl) {
            showAlert('warning', 'Veuillez saisir au moins une URL ou utiliser des fichiers locaux.');
            return;
        }
    }
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                showAlert('danger', 'Erreur lors de l\'ajout de la source: ' + (response.error || 'Une erreur inconnue est survenue.'));
            }
        },
        error: function(xhr, status, error) {
...
function editSource(index) {
    ...
    $.ajax({
        ...
        success: function(config) {
            ...
            $('#edit-use-local-files').prop('checked', source.use_local_files || false);
            toggleEditUrlInputs(source.use_local_files || false);
            $('#edit-test-results').addClass('d-none').empty();

            // Show edit modal
            const editSourceModal = new bootstrap.Modal(document.getElementById('edit-source-modal'));
...
function updateSource() {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                // Close modal
                bootstrap.Modal.getInstance(document.getElementById('edit-source-modal')).hide();
                // Reload config
                loadConfig();
                showAlert('success', 'Source mise à jour avec succès.');
            } else {
                showAlert('danger', 'Erreur lors de la mise à jour de la source: ' + (response.error || 'Une erreur inconnue est survenue.'));
            }
        },
        error: function(xhr, status, error) {
            showAlert('danger', 'Erreur lors de la mise à jour de la source: ' + error);
        }
    });
}
...
function confirmRemoveSource(index) {
    ...
    const confirmModal = new bootstrap.Modal(document.getElementById('confirm-remove-modal'));
    confirmModal.show();

    // Set confirm button action
    $('#confirm-remove-btn').one('click', function() {
        removeSource(index);
        confirmModal.hide();
    });
}
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                showAlert('danger', 'Erreur lors de la suppression de la source: ' + (response.error || 'Une erreur inconnue est survenue.'));
            }
        },
        error: function(xhr, status, error) {
            showAlert('danger', 'Erreur lors de la suppression de la source: ' + error);
        }
    });
}
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                showAlert('danger', 'Erreur lors de la mise à jour de la source active: ' + (response.error || 'Une erreur inconnue est survenue.'));
            }
        },
        error: function(xhr, status, error) {
            showAlert('danger', 'Erreur lors de la mise à jour de la source active: ' + error);
        }
    });
}
...
function refreshData() {
    ...
    const overlay = $('<div class="loading-overlay"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Chargement...</span></div></div>');
    $('body').append(overlay);
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.success) {
                ...
                showAlert('danger', 'Erreur lors du rafraîchissement des données: ' + (response.error || 'Une erreur inconnue est survenue.'));
            }
        },
        error: function(xhr, status, error) {
            // Remove loading overlay
            $('.loading-overlay').remove();
            showAlert('danger', 'Erreur lors du rafraîchissement des données: ' + error);
        }
    });
}
